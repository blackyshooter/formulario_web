const $ = (id) => document.getElementById(id);
  ...
  // ====== NUEVO: traer "Último material" desde PostgreSQL via mini-API ======
  async function cargarUltimoMaterialDesdeDB(){
    try {
      // Ajustá el puerto si tu API corre en otro
      const r = await fetch("http://127.0.0.1:5050/api/ultimo-material", { cache: "no-store" });
      const data = await r.json();

      if(!data?.ok || !data?.material_id){
        setStatus("⚠️ No se pudo obtener el último material (sin datos).", "bad");
        return;
      }

      // Normalizamos a 18 dígitos (tu sistema trabaja con 18)
      const mat18 = pad18(String(data.material_id).replace(/\D/g, ""));
      $("ultimoMat").value = mat18;

      setStatus(`✅ Último material desde DB: ${mat18}`, "ok");
    } catch (e) {
      console.error(e);
      setStatus("❌ Error consultando el último material (API no disponible).", "bad");
    }
  }
